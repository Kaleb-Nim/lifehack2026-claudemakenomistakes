...
Last activity: 2026-08-30 — **Quick task: the agent now takes a beat before replying** (`QUICK-agent-thinking-time.md`, ahead of 02-03). `lib/beat-runner.ts`'s `sendResponseCreate` (still the sole `response.create` send site) now opens a think pause before it sends: `BEAT_THINK_MS` (700ms, orb-only `thinking` state) on every ordinary turn, `WORK_THINK_MS` (4500ms, orb + progress bar + `WORK_THINK_TRACE` trace lines) only on beat C's post-upload "reading the sources" beat via a new per-beat `thinkTier`. `AUDIO_TIMEOUT_MS` starts only once the pause elapses and the send actually happens; a fresh `speech_started` cancels a pending pause outright (never resumes it) so the agent never answers a stale turn; think state force-clears the instant the session leaves "live" so the orb can never stick. `components/Onboarding.tsx` + `app/globals.css` add a distinct `.orb.thinking` pulse (not the full `.orb.live` treatment) and a `.think-panel` for the Work tier only. One holding-line sentence added to `lib/agent-context.md`'s "How you speak", scoped to the reading moment. UI-SPEC orb-state table gets a new State 3.5 row. tsc/lint/34 tests clean; `bun run build` clean. Total added time across a full A→G take: ~5 Beat-tier pauses (~3.5s) + 1 Work-tier pause (4.5s) ≈ 8s against the 150s frame budget. Earlier: **Quick task: owner caption gets a 3-turn rolling history** (`QUICK-caption-history.md`, ahead of 02-03). `lib/beat-runner.ts` now keeps up to 3 finalized owner turns (oldest first) alongside the in-progress caption, gated by the same MIN_SPEECH_MS threshold as the visible caption, pushed on the authoritative `…input_audio_transcription.completed` event; survives a frame advance, clears on disconnect/new session. `components/Onboarding.tsx` + `app/globals.css` render the stack (newest at bottom, older entries dimmer/smaller) in real-session mode only — `?mode=scripted` is byte-identical to before. UI-SPEC §2 Finalization row amended to match. tsc/lint/31 tests clean. 02-03's caption assumptions are stale; re-read the QUICK doc when it resumes. Earlier: **Phase 6 (Neo4j graph) removed entirely** — ROADMAP section + progress row gone, GRAPH-01…03 struck from REQUIREMENTS (34 → 31 reqs) and recorded under Out of Scope, PROJECT.md architecture now ends at the on-screen listing. Earlier: **all hardcoded demo data aligned to `docs/CANONICAL-DEMO-DATA.md`** (user-directed, out-of-band — NOT plan 02-02): catalogue is now the canonical §3 ten items with matching prices/stock, hero card is the ASUS Vivobook 15 spine product, every on-screen count derives from one `CATALOGUE_COUNT` export, and the product name is settled as **Cashew** in code *and* in the canonical doc (§1 flipped from Pluto). `lib/agent-context.md` renamed to Cashew and its shelf-photo counts corrected. tsc/lint/4 tests clean; states C, D and G screenshot-verified. Earlier: 02-02 re-planned around the context bias (verbatim original archived as superseded; REQUIREMENTS SCRIPT-02 -> SCRIPT-02a, VOICE-03/04 and SCRIPT-01 amended; ROADMAP Phase 2 goal and criteria re-scoped). Earlier: 02-01 tracer merged (31d9b83); trigger fixes 34a85cf; PRODUCT_NAME -> Pluto f78245e; context-bias decision recorded. Earlier: Phases re-ordered: GPT Realtime voice (real speech both ways, scripted lines + canned tool results) comes before uploads and recording; real brain and Neo4j after the video. Needs `OPENAI_API_KEY` in `apps/merchant/.env.local`.
